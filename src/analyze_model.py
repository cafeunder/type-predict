...
def main():
    ...
    for poke_name in poke_names:
        ...
        for image in image_list:
            # 画像からタイプを予測
            if args.gpu >= 0:
                y_type1 = F.softmax(model_type1.predictor(chainer.cuda.cupy.array([preprocess_image(image, mean1, 224)])))
                y_type1 = chainer.cuda.cupy.argmax(y_type1.data)
                y_type2 = F.softmax(model_type2.predictor(chainer.cuda.cupy.array([preprocess_image(image, mean2, 224)])))
                y_type2 = chainer.cuda.cupy.argmax(y_type2.data)
            else:
